...
>
    {#if !isEditing}
        <div class="form-control mb-3">
            <label for="asset-code" class="label"><span class="label-text">Mã Tài sản</span></label>
            <input id="asset-code" type="text" value="Tự động tạo khi lưu" readonly class="input input-bordered w-full bg-slate-100 text-slate-500 italic" />
        </div>
    {:else if formData.code}
        <div class="form-control mb-3">
            <label for="asset-code-edit" class="label"><span class="label-text">Mã Tài sản</span></label>
            <input id="asset-code-edit" type="text" value={formData.code} readonly class="input input-bordered w-full bg-slate-100 font-bold" />
        </div>
    {/if}

    <div class="form-control w-full mb-3">
        <label for="asset-name" class="label">Tên Tài sản</label>
        <input id="asset-name" type="text" bind:value={formData.name} class="input input-bordered w-full" placeholder="VD: Lò nướng Sanaky" />
    </div>
    <div class="grid grid-cols-2 gap-4 mb-3">
        <div class="form-control">
            <label for="asset-category" class="label">Loại</label>
            <select id="asset-category" bind:value={formData.category} class="select select-bordered">
        ...
        <div class="form-control">
            <label for="asset-price" class="label">Giá mua (đ)</label>
            <input id="asset-price" type="number" bind:value={formData.originalPrice} class="input input-bordered" />
        </div>
    </div>
    <div class="grid grid-cols-3 gap-4 mb-3 bg-base-200 p-4 rounded-box">
        <div class="form-control">
            <label for="qty-good" class="label text-success">Tốt</label>
            <input id="qty-good" type="number" bind:value={formData.quantity.good} class="input input-bordered input-sm" />
        </div>
        <div class="form-control">
            <label for="qty-broken" class="label text-warning">Hỏng</label>
            <input id="qty-broken" type="number" bind:value={formData.quantity.broken} class="input input-bordered input-sm" />
        </div>
        <div class="form-control">
            <label for="qty-lost" class="label text-error">Mất</label>
            <input id="qty-lost" type="number" bind:value={formData.quantity.lost} class="input input-bordered input-sm" />
        </div>
    </div>
</Modal>
