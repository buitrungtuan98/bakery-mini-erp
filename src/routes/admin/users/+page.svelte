                    ...
                    {#each users as user}
                        <div class="bg-base-100 p-4 rounded-lg shadow-sm border border-base-200">
                            ...
                            <div class="form-control">
                                <label class="label pt-0"><span class="label-text text-xs uppercase font-bold">Vai trò</span></label>
                                <select class="select select-bordered select-sm w-full" bind:value={user.role} on:change={(e) => updateUserRole(user, e.currentTarget.value)} disabled={user.id === $authStore.user?.uid}>
...
<Modal title="Mời thành viên mới" isOpen={isInviteModalOpen} onConfirm={handleInvite} onCancel={() => isInviteModalOpen = false} loading={inviteLoading} confirmText="Gửi lời mời">
    ...
    <div class="form-control w-full">
        <label class="label"><span class="label-text">Vai trò</span></label>
